...
// SafuSwap's rewards are generated by swapping rewards to SAFU and sending to bar address for distribution
contract SafuMakerV2 is Ownable {
    ...
    constructor(address _factory, address _bar, address _safu, address _usdc) {
    ...
    function convertMultiple(
        ...
        require(
            ...
            "SafuMakerV2: token lengths don't match"
        );
    ...
    // @audit is this for the swap router?
    ...
    function _convert(address token0, address token1) internal {
        ...
        (uint amount0b, uint amount1b) = pair.burn(address(this));
        // We get the amount0 and amount1 by their respective balance of SafuMakerV2.
        uint256 amount0 = IERC20(token0).balanceOf(address(this));
        uint256 amount1 = IERC20(token1).balanceOf(address(this));
        uint safuOut = _convertStep(token0, token1, amount0, amount1);
    }
    ...
    function _convertStep(
        ...
        if (token0 == token1) {
            ...
            safuOut = _toSafu(
                usdc,
                _swap(token0, usdc, amount0, address(this)) + (amount1)
            );
        } else {
    ...
    function setBridge(address token, address bridge) external onlyOwner {
        require(
            token != safu && token != usdc && token != bridge,
            "SafuMakerV2: Invalid bridge"
        );
